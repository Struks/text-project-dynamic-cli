<template>
    <div class="margin-bottom">
        ...
        <div class="container text-center">
            <div >
                <img v-if="!currentUser.img"  class = "rounded-circle img" :src='noimg' alt = "no image" width="400px"> 
                <img v-if="currentUser.img"  class = "rounded-circle img" :src='currentUser.img' alt = "user image" width="350px"> 
            </div> 
            <h1 class="mt-3 mb-5">{{currentUser.firstname}} {{currentUser.lastname}}</h1>
            <h4>BIO:</h4>
            <p class="mb-5" v-html="currentUser.bio"></p>
                <button type="button" class="btn btn-success" @click="editProfil(currentUser.id)">EDIT PROFIL</button>
        </div> 
    </div>
</template> 
...
export default {
    ...
    computed:{
        currentUser(){return this.$store.getters.logUser}
    },
    methods:{
        editProfil(id){
